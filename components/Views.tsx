...
import { PROJECTS, TOOLS, ARTICLES, DESIGNS } from '../constants';
import { Box, Code, Cpu, Eye, Layout, Terminal, Clock, X, Calendar, Globe, Server, Shield, Activity, Layers } from 'lucide-react';
import { DesignItem, Category } from '../types';

const CardHover: React.FC<React.PropsWithChildren<unknown>> = ({ children }) => (
...
// --- Swapped & Renamed Views ---

// Formerly HomeView (Text Content), now ProfileView (About Section)
...
// Formerly AboutView (Splash), now SplashView (Home Section)
export const SplashView: React.FC<{ onNavigate: (c: Category) => void }> = ({ onNavigate }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center animate-fade-in">
       ...
       <div className="flex gap-6">
         <button onClick={() => onNavigate('webpages')} className="px-8 py-4 rounded-full bg-white text-black font-medium text-sm hover:bg-zinc-200 transition-colors tracking-widest uppercase">
           View Work
         </button>
         <button onClick={() => onNavigate('about')} className="px-8 py-4 rounded-full border border-white/20 text-white font-medium text-sm hover:bg-white/10 transition-colors tracking-widest uppercase">
           Operator Dossier
         </button>
       </div>
    </div>
        ...
        <CardHover key={project.id}>
            <div className="aspect-video relative overflow-hidden">
              ...
              <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full border border-white/10">
                 <span className="text-[10px] uppercase tracking-wider text-white">{project.category}</span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {TOOLS.map((tool) => (
          <div key={tool.id} className="bg-surface border border-white/5 p-6 hover:border-amber-400/50 hover:bg-white/5 transition-all duration-300 rounded-xl group flex flex-col justify-between h-40">
            ...
            <div>
              <h4 className="text-base font-medium text-white mb-1">{tool.name}</h4>
              <p className="text-[10px] text-zinc-500 line-clamp-2">{tool.description}</p>
            </div>
          </div>
        ))}
      </div>

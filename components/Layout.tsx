                ...
                <div className="grid grid-cols-7 gap-2">
                  {Array.from({length: 30}, (_, i) => (
                    <div 
                      ...
                      ${i + 1 === new Date().getDate() ? 'bg-white text-black font-bold scale-110 shadow-lg' : 'text-zinc-400 hover:bg-white/10 cursor-pointer'}`}
                    >
                      {i + 1}
                    </div>
                  ))}
...
export const Dock: React.FC<DockProps> = ({ activeCategory, onNavigate }) => {
  const items = [
    { id: 'webpages', label: 'Webpages', icon: Layout, color: 'text-pink-400' },
    { id: 'webapps', label: 'Webapps', icon: Cpu, color: 'text-blue-400' },
    { id: 'webtools', label: 'Webtools', icon: Wrench, color: 'text-amber-400' },
    { id: 'articles', label: 'Articles', icon: FileText, color: 'text-emerald-400' },
  ];
      ...
      <nav className="pointer-events-auto flex items-center gap-4 px-6 py-4 bg-black/30 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl transition-all duration-300 hover:scale-105 hover:bg-black/40">
        {items.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => onNavigate(item.id as Category)}
              className={`
